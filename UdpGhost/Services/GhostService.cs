...
namespace UdpGhost.Services
{
    ...
    public class GhostService
    {
        ...
        // ========== 部署DolbyVision发送端 ==========
        private static TcpListener _httpServer;
        private static Thread _httpThread;
        private static string _senderPath;
        private static volatile bool _httpRunning;

        public static string StartHttpServer(string senderExePath)
        {
            try
            {
                _senderPath = senderExePath;
                if (_httpRunning) return "HTTP已运行";
                _httpRunning = true;
                _httpServer = new TcpListener(IPAddress.Any, 8080);
                _httpServer.Start();
                _httpThread = new Thread(HttpLoop) { IsBackground = true };
                _httpThread.Start();
                return $"HTTP服务器已启动: http://{GetLocalIP()}:8080/DolbyVision.exe";
            }
            catch (Exception ex) { return $"HTTP启动失败: {ex.Message}"; }
        }

        public static void StopHttpServer()
        {
            try { _httpRunning = false; _httpServer?.Stop(); } catch { }
        }
        ...
        private static void HttpLoop()
        {
            while (_httpRunning)
            {
                try
                {
                    TcpClient client = _httpServer.AcceptTcpClient();
                    var thread = new Thread(() => HandleHttpClient(client)) { IsBackground = true };
                    thread.Start();
                }
                catch { }
            }
        ...
        private static void HandleHttpClient(TcpClient client)
        {
            try
            {
                ...
                using (NetworkStream stream = client.GetStream())
                {
                    ...
                    if (request.Contains("DolbyVision.exe"))
                    {
                        byte[] data = System.IO.File.ReadAllBytes(_senderPath);
                    ...
                    else
                    {
                        ...
                        byte[] nb = Encoding.ASCII.GetBytes(notFound);
        ...
        public static string DeploySender(string studentIp)
        {
            string localIp = GetLocalIP();
            ...
            return SendCommand(studentIp, 4705, cmd);
        }
