...
### 版本信息
- **版本号**：QD_V2.7_JiYuRebuild_FakeMonitor
- **发布日期**：2026-09-14
- **Commit**：8478895（主项目）/ 76ce884（DLL）
...
### 视频预览功能
- **MediaElement预览**：从静态Image改为MediaElement，支持视频预览
- **默认不自动播放**：选择视频后显示第一帧，点"播放"才开始
- **播放/暂停按钮**：可控制预览播放
- **循环播放**：视频结束后自动循环
- **已静音**：`IsMuted="True"`避免音画不同步问题
- **按钮UI修复**：高度32、宽度64、Padding=8,0，文字不被遮挡
...
### 待办事项
- **视频预览主程序卡顿**：32位MediaElement性能问题，暂不修复
- **32位/64位适配**：正式机房使用需要识别系统位数，32位用32位DLL+ffmpeg，64位用64位DLL+ffmpeg
- **GDI hook方向研究**：作为长期待办，当前编码层方案已满足需求
