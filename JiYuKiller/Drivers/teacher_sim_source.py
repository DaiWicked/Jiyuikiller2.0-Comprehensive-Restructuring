...
# 工作目录: exe所在目录/teacher_sim/
_base_dir = os.path.dirname(os.path.abspath(sys.argv[0])) if getattr(sys, 'frozen', False) else os.path.dirname(os.path.abspath(__file__))
LOG_DIR = os.path.join(_base_dir, 'teacher_sim')
...
def handle_tnal(d, sip):
    ...
    if state['got'] >= total:
        # 按学生IP建立子文件夹
        student_dir = os.path.join(LOG_DIR, sip.replace('.', '_'))
        os.makedirs(student_dir, exist_ok=True)
        idx = 0
        while True:
            fn = os.path.join(student_dir, f'preview_{idx}.jpg')
    ...
    while running:
        try:
            ...
            if mag == 0x49474F4C:  # LOGI
                ...
                # 被动模式: 不自动请求预览, 等待用户手动输入 preview <ip>
...
def _check_single_instance():
    ...
    try:
        ...
        return True  # 检测失败不阻止启动


def _check_port_occupied():
    """检测教师端端口是否已被占用（PORT和SPORT）"""
    occupied = []
        ...
        while time.time() < deadline:
            try:
                ...
                if len(data) >= 4:
                    ...
                    # NANC=0x434E414E, CANC=0x434E4143, OONC=0x434E4F4F 都是教师宣告
